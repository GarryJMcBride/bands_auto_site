...
- [ ] Fix static file paths for FastAPI:
  ...
  - Ensure CSS, JS, and images resolve properly when served via FastAPI
    → Reference: _“relative paths in fastapi” + your browser notes_
...
- [ ] Install and configure:
  ...
  - [x] `ruff` (linting) — installed as a `uv` dependency group (`pyproject.toml` `[dependency-groups] lint`), configured via `[tool.ruff.lint] select = [...]`, wired into global VS Code `settings.json` (`[python]` `defaultFormatter` + `editor.codeActionsOnSave`), documented in `README.md`
  - [ ] `pylance` (type checking / IntelliSense)
  - [x] `prettier` — installed as an npm devDependency, configured via `.prettierrc`/`.prettierignore`, `npm run format` script scoped to `src/frontend/typescript/**/*.ts`, documented in `README.md`
...
[x] Change all reference to "Quote" for the "Book in now" submission, its called "Quote" from the template, not its use case on B&S Autos — renamed across schemas/models/routers/mailer/TypeScript (`BookSubmission`, `handle_book_inputs.py`, `bookForm.ts`, `/api/book-javascript-pipeline`, `/book-python-pipeline`, `book_submissions` table, `?book_submitted=`). Left the theme's own HTML/CSS class names (e.g. `get-quote-section`) unchanged since those are presentational, from the prebuilt Linoor template.
...
`node_modules/dompurify/dist/purify.es.mjs`to`static/js/vendor/dompurify/purify.es.mjs`and wired via an import map in`index.html`, because there`s no bundler to resolve `node_modules` packages
...
- [ ] Add ESLint for TypeScript import sorting/cleanup — Prettier only formats
      style (quotes/spacing), it doesn't sort import blocks or flag unused
      imports the way Ruff's `I`/`F` rules do for Python; nothing currently
      does this for `.ts` files. Likely `eslint-plugin-simple-import-sort` or
...
- [ ] Reduce CSS bloat:
  ...
  - See `docs/frontend-bloat-audit.md` → "3. CSS bloat audit" for the specific, checked-off list of candidates (rtl.css, jquery-ui.css, unused theme sections in style.scss, etc.)
...
- [ ] Security considerations:
  ...
  - Ensure safe handling of vehicle data
    _(Note: Reg lookups are public, but API usage must still be secured)_
...
- [ ] Add honeypot field or rate limiting on the form endpoint to stop spam - Add a honeypot field to the form schema (hidden field bots fill in, humans don`t —
...
- [ ] Add an end-to-end test proving the no-JS `/book-python-pipeline` fallback
...
## 13. Frontend Bloat Audit — Remove unused Envato/Linoor template CSS/JS

- [ ] Work through the full checklist in `docs/frontend-bloat-audit.md` and
      remove confirmed-dead CSS/JS (unused vendored plugins like `mixitup.js`,
      `knob.js`, `appear.js`, `jquery.fancybox.js`, `isotope.js`, `nouislider.*`,
      plus unused CSS like `rtl.css`) — that doc has the specific files, line
      numbers, and verification steps already worked out, don`t duplicate them
      here, just track completion.
- [ ] Priority item inside that audit: verify whether the leftover
      `$("#contact-form").validate({...})` call in `custom-script.js` (jQuery
      Validation Plugin) is interfering with `enquiryForm.ts`'s own submit
      handling on the real enquiry form — see "Priority finding" section of the
      audit doc.

## 14. Upgrade jQuery UI to a patched version (security)

- [ ] `jquery-ui.js` is v1.12.1 (2016) with known XSS CVEs — see
      `docs/architecture.md`. Core `jquery.js` (3.5.0) is already patched; this
      is the one outdated dependency actually worth a security-motivated upgrade.
- [ ] Only usage in the codebase is `.selectmenu()` on the booking form`s
service dropdown (`custom-script.js`, `.custom-select-box`/`index.html`'s `<select name="service">`) — small blast radius, so this
      should be a contained change.
- [ ] Upgrade to whatever current jQuery UI release has these CVEs patched
      and confirm `.selectmenu()`'s API/markup is unchanged, OR replace the
      widget with a lighter modern alternative if no simple drop-in upgrade
      exists (native `<select>` + custom CSS, Alpine.js, Floating UI, etc. — see
      `docs/architecture.md`'s "Modern alternatives" list).
- [ ] Remove the dead `.datepicker()` init in `custom-script.js` regardless
      of the above (no `.date-picker` element exists on the page) — see
      `docs/frontend-bloat-audit.md` section 1.
