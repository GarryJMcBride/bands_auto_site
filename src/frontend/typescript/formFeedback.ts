...
export function displayErrors<T>(
  ...
  for (const [field, message] of Object.entries(errors as Record<string, string>)) {
...
function submitMessageId(form: HTMLFormElement): string {
 ...
 */
export function showSubmitMessage(
  form: HTMLFormElement,
  type: 'success' | 'error',
  message: string,
): void {
...
export function clearSubmitMessage(form: HTMLFormElement): void {
