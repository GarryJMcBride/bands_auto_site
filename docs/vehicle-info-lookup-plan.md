...
The booking form already collects and validates a UK vehicle registration
(`BookSubmission.registration` in `src/backend/schemas.py`). The goal is to
...
unconfigured, or returns an error, the booking submission must still save and
...
- `src/backend/vehicle_lookup.py` — `async def gather_vehicle_details(registration: str) -> dict`, the only one of the three the router imports. Runs both providers concurrently via `asyncio.gather(..., return_exceptions=True)` and always returns `{"dvla": dict | None, "mot_history": dict | None}`, never raising, never `None` — this is the seam that keeps the router ignorant of DVLA/DVSA specifics, mirroring how it already treats `mailer.send_book_email` as a black box.
...
**Database** — `CREATE_BOOK_TABLE_SQL` (`schemas.py`) gets a new nullable
...
`schemas.MIGRATIONS_SQL = ["ALTER TABLE book_submissions ADD COLUMN IF NOT EXISTS vehicle_details JSONB;"]`
...
`await db.ensure_schema([schemas.CREATE_BOOK_TABLE_SQL, *schemas.MIGRATIONS_SQL])`.
...
submission_id = await update_book_database(payload, vehicle_details)
...
await send_book_email(payload, submission_id, vehicle_details)
```

`save_book`/`update_book_database` gain a `vehicle_details: dict` parameter,
...
contractually non-raising. `BookSubmission` itself is **not** changed
(vehicle_details is server-fetched enrichment, not user input), so no
`Form(...)` field or client-side change is needed.

**Email** (`mailer.py`) — `build_book_email_body`/`build_book_email_html`/`send_book_email`
...
- `src/backend/schemas.py` — `CREATE_BOOK_TABLE_SQL` column + new `MIGRATIONS_SQL`.
- `src/backend/classes.py` — `Database.ensure_schema` accepts `str | list[str]`.
- `src/backend/app.py` — `lifespan()` call site update.
- `src/backend/mailer.py` — `_vehicle_detail_rows` + threaded `vehicle_details` param on all three booking functions.
- `src/backend/routers/handle_book_inputs.py` — `gather_vehicle_details` call + threaded param on `save_book`/`update_book_database` and both endpoints.
...
4. Restart the app against a Postgres DB created _before_ this change, to confirm the new `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` migration step runs cleanly with no manual intervention.
