...
- `transferFormInput.ts` had a hardcoded `http://127.0.0.1:8000/api/quote-javascript-pipeline` fetch target, a different origin than the app's actual `localhost:8000`, which CORS (no `allow_origins` configured) would have blocked anyway. Now a relative `/api/quote-javascript-pipeline`.
...
- [x] Submitted the form with JS enabled — confirmed `fetch` POST to `/api/quote-javascript-pipeline`, no page reload, success banner shown, row saved to Postgres.
- [x] Submitted the form with JS disabled — confirmed native POST to `/quote-python-pipeline`, redirect with `?submitted=success`, same validation/save/email path.
